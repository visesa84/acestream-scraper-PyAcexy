...
class BaseScraper(ABC):
    ...
    def clean_channel_name(self, name: str) -> str:
        ...
        # 1. Eliminar el formato largo: " ID_HEX --> ORIGEN" (ej: 363d --> ELCANO)
        name = re.sub(r"\s+[0-9a-fA-F]{4}\s+-->\s+.*$", "", name)
        
        # 2. Eliminar flechas sueltas y lo que sigue
        if " --> " in name:
            name = name.split(" --> ")[0]

        # 3. Eliminar asteriscos (uno o más) al final
        name = re.sub(r"\*+$", "", name)

        # 4. Eliminar etiquetas de calidad comunes al final (opcional)
        # Esto quita: 1080p, 720p, FHD, HD, SD, 4K
        #name = re.sub(r"\s+(1080p|720p|fhd|hd|sd|4k|uhd)$", "", name, flags=re.IGNORECASE)

        # 5. Limpieza final de espacios y caracteres de puntuación residuales
